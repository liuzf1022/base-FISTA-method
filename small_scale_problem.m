...
% 调用FISTA_1方法
[K, x_min, f_min, X, F, dF_norm] = FISTA_1_method([-10; -1], 1e-3,A,b);
disp(['利用FISTA_1方法: 迭代次数 = ', num2str(K), ', 最优值点 x1 = ', num2str(x_min(1)), ', x2 = ', num2str(x_min(2)), ', 最小值 = ', num2str(f_min)]);

% 调用等步长梯度下降法
[K1, x_min1, f_min1, X1, F1, dF_norm1] = GD_method([-10; -1], 1e-3,A,b);
...
% FISTA_1 方法
function [K, x_min, f_min, X, F, dF_norm] = FISTA_1_method(start, eps,A,b)
...
% 等步长梯度下降法
function [K, x_min, f_min, X, F, dF_norm] = GD_method(start, eps,A,b)
...
function Go_plot( X1, F1, dF_norm1, X2, F2, dF_norm2,A,b)
    ...
    legend([p1 p2 p3], {'FISTA','等步长梯度下降法','最优解'},...
