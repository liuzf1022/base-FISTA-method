...
% 调用FISTA_1方法
[K, x_min, f_min, X, F, dF_norm] = FISTA_1_method(ones(n,1), 1e-3,A,b,m,n);
disp(['利用FISTA_1方法: 迭代次数 = ', num2str(K), ', 最优值点 x1 = ', num2str(x_min(1)), ', x2 = ', num2str(x_min(2)), ', 最小值 = ', num2str(f_min)]);

% 等步长梯度下降法
[K1, x_min1, f_min1, X1, F1, dF_norm1] = GD_method(ones(n,1), 1e-3,A,b,m,n);
disp(['利用等步长梯度下降法：迭代次数 = ', num2str(K1), ', 最优值点 x1 = ', num2str(x_min1(1)), ', x2 = ', num2str(x_min1(2)), ', 最小值 = ', num2str(f_min1)]);

% 绘制图形
Go_plot( X, F, dF_norm, X1, F1, dF_norm1,A,b);

% FISTA_1 方法
function [K, x_min, f_min, X, F, dF_norm] = FISTA_1_method(start, eps,A,b,m,n)
...
% 等步长梯度下降法
function [K, x_min, f_min, X, F, dF_norm] = GD_method(start, eps,A,b,m,n)
...
function Go_plot( X1, F1, dF_norm1, X2, F2, dF_norm2,A,b)
    ...
    % legend([p1 p2 p3], {'FISTA','最速下降法','最优解'},...
    ...
    legend({'FISTA','等步长梯度下降法'}, 'Location', 'best', 'FontSize', 10)
